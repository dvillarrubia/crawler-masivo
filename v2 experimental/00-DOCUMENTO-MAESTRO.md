...
- [x] **B1 · API para agentes — COMPLETO** — (1) **Servidor MCP** `mcp_server/` (capa fina sobre la REST, solo httpx): 10 verbos ES (listar_proyectos/rastreos, lanzar/estado/resumen/cancelar_rastreo, top_incidencias, buscar_urls, detalle_url, preguntar_a_los_datos). Lógica en `verbs.py` (testeable sin el SDK) + `server.py` FastMCP. `claude mcp add crawler -- python -m mcp_server.server`. (2) **Auth por API key por proyecto** `api/auth.py` (`ApiKeyAuthMiddleware`, OFF por defecto → retrocompatible; scoping por client_id; tabla `api_keys` solo-hash; gestión con `ADMIN_TOKEN`; router `api_keys.py`; MCP manda `CRAWLER_API_KEY`) + UI en Cuentas y fuentes. 19 tests (10 MCP + 9 auth). Verificado en vivo (auth off intacto; auth on 401/200/403 correcto). Commits: (b1 mcp), (b1 auth), (b1 ui)
