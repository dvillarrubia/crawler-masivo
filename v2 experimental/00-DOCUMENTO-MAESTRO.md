...
- [x] **3.3** Vistas sobre API existente: jobs + nuevo rastreo (modal Empresarial) + progreso en vivo + overview + **explorador fusionado** + ficha de URL (drawer) + técnico/on-page. — PARCIAL: semántica y cuentas GSC/Gemini siguen en `/legacy` (enlazadas desde sidebar y Configuración); re-skin pendiente. Prefs de columnas y navegación por teclado del explorador legacy aún no portadas (2026-07-04)
...
- [x] **7.2 T18** PageRank semántico (NÚCLEO) — `compute_semantic_pagerank` en link_suggester (peso arista = pos × (0.3 + 0.7×cos), misma power iteration T17.1, `urls.pagerank_semantic`), post-paso del análisis semántico; `GET /jobs/{id}/pagerank-delta` ordenado por |delta| con blocked explícito. PENDIENTE de T18: relevancia de anchors (`embed_query` por anchor, `generic_anchor`/`anchor_target_mismatch`) y anchor propuesto en sugerencias T10 (2026-07-04)
- [x] **7.3 T21** simulador what-if — `POST /jobs/{id}/pagerank-simulate` (add/remove hasta 500 mutaciones, grafo v2 en memoria, baseline y mutado con la misma power iteration compartida, cero escrituras — pureza verificada por test); nota: la escala 0-10 normalizada hace que la página máxima tenga delta 0 por construcción. PENDIENTE: caché de matriz por job para simulaciones interactivas (2026-07-04)
- [ ] **7.4 T19** cobertura consulta→pasaje (T9+T11) — PENDIENTE: requiere embed_query de queries GSC en runtime (tabla `query_embeddings`, matriz query×chunk vía HNSW, gap de pasaje / pasaje enterrado / chunk huérfano como issues firmables). La infraestructura previa (chunks persistidos T11, GSC con url_hash T9, patrón de firma T10) ya está toda.
